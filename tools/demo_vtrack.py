...
def make_parser():
    ...
    parser.add_argument(
        "--ymin", default=None, type=int, help="minimum cutoff y for bounding boxes"
    )
    parser.add_argument(
        "--max_plays", default=None, type=int, help="max plays"
    )
    ...
    parser.add_argument('--img-size', default=None,
                        help='img size (w,h) passed to model. Default is to maintain resolution')
    parser.add_argument(
        "--device",
        default="gpu",
        type=str,
    ...
    parser.add_argument("--tsize", default=None, help="test img size: h,w")
...
class Predictor(object):
    ...
    def inference(self, img, timer):
        ...
        with torch.no_grad():
            ...
            outputs = postprocess(
                outputs, self.num_classes, self.confthre, self.nmsthre, self.ymin
            )
...
def imageflow_demo(dataloader, predictor, current_time, args, result_filename, vid_writer,
        ...
        if outputs[0] is not None:
            ...
            for t in online_targets:
                ...
                if tlwh[2] * tlwh[3] > args.min_box_area and not vertical:
                    ...
                    online_scores.append(t.score)
                    results.append(
                        f'{vid_fnum},{tid},{tlwh[0]:.2f},{tlwh[1]:.2f},{tlwh[2]:.2f},{tlwh[3]:.2f},{play_num},-1,-1,-1\n'
                    )
            timer.toc()
            online_im = plot_tracking(
    ...
    if args.save_result:
        with open(result_filename, 'w') as f:
            f.writelines(results)
        logger.info(f"save results to {result_filename}")


def main(exp, args):
    result_root = osp.join(cfg.output_root, 'ByteTrack', args.match_name)
    if args.tag is not None:
        result_root += f'_{args.tag}'
    os.makedirs(result_root, exist_ok=True)

    args.ckpt = osp.join(cfg.checkpoints, 'ByteTrack', args.ckpt)
    ...
    if args.tsize is not None:
        exp.test_size = list(int(x) for x in args.tsize.split(','))
    ...
    dataloader = LoadVideo(mobj.vid_fn,
                           args.img_size,
                           play_seconds=mobj.play_seconds,
                           vid_sec_to_play=mobj.vid_sec_to_play)
